...
if ( $available_methods ) {
	...
	// Show radio buttons for methods
		...
		echo '<ul id="shipping_method" style="list-style:none;">';
		echo '<style>
                          ul li .amount {float:right; font-weight:bold}
                   </style>';
		foreach ( $available_methods as $method )
			echo '<li style="text-align:left;"><input type="radio" name="shipping_method" id="shipping_method_' . sanitize_title( $method->id ) . '" value="' . esc_attr( $method->id ) . '" ' . checked( $method->id, $woocommerce->session->chosen_shipping_method, false) . ' style="width:20px; margin-right:10px;" /> <label for="shipping_method_' . sanitize_title( $method->id ) . '">' . wp_kses_post( $method->full_label ) . '</label></li>';
